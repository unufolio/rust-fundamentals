...
fn borrow_vec(mut vector: &Vec<i32>) {
    // `vector` is a `&` reference, so the data it refers to cannot be borrowed as mutable
    // vector.push(10);
    println!("{:?}", vector);
}
...
fn main() {
    ...
    let mut my_vec_for_borrow = vec![1, 2, 3, 4, 5];
    let my_int = 10;
    let my_string = String::from("Hello, world!");
    let my_string_for_borrow = String::from("Hello, world!");
    ...
    borrow_string(&my_string_for_borrow);
    println!("after borrowed {:?}", my_string_for_borrow);

    own_vec(my_vec);
    borrow_vec(&my_vec_for_borrow);
    println!("after borrowed {:?}", my_vec_for_borrow);
